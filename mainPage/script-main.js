  ...
  fetchData
    ...
    .then((data) => {
      ...
      if (!data.choices || !data.choices[0]) {
        ...
        throw new Error("AI response error: " + (data.error?.message || "Unknown error"));
      }

      const resultData =  data.choices[0].message.content;

      messageHistory.push({ role: "assistant", content: resultData }); isAnswerLoading = false; addAnswerSection(resultData);
    })
    .finally(() => {
      scrollToButtom();
